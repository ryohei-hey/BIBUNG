BIBUNG「美しき我が日本語を汚す悪文は、何文たりとも許さない。」
...
科学的忠実性、論理の明晰さ、日本語の自然さ、簡潔さの順に優先します。修正範囲の指定がなければ、段落の順序や構成から直します。パラグラフ・ライティングを基本とします。原文の箇条書き・表は保ち、文章中の列挙を箇条書きや表にするのは、用途と書式が許す場合に限ります。
...
語句が正確なのに読みにくい文章では、動作が名詞になっていることが多いです。動作を名詞にすると、誰が何をしたのかを書かなくても日本語の文は成立します。書かれなかった部分は読者が前の文から補うことになるため、語句をいくら見直しても誤りは見つからず、読みにくさだけが残ります。次は独自作成した架空例です。
...
> 退院支援の質は、退院前カンファレンスの職種構成に左右される。医師と看護師だけでカンファレンスを構成すると、計画は医学的な課題に偏りやすい。これに対し、薬剤師と理学療法士を加えた職種構成では、生活面の課題も計画に入る。A病院と当院が後者の職種構成を採っているのは、これにより再入院が減ることを期待しているためである。
...
どれも原文にある語を使い直しただけで、新しい事実は足していません。誰が何をしたのかは、いずれも同じ段落に書かれていました。型の一覧と、それぞれの「直さない場合」は[日本語の編集基準](references/japanese-editing.md)にあります。
...
`~` はユーザーのホームフォルダです。配置後、各フォルダの直下に `SKILL.md` と `references/` があることを確認し、新しいセッションで呼び出してください。既存の `bibung` フォルダがある場合は、変更を確認してから更新します。配置は環境ごとに必要です。claude.aiに追加したスキルは、CodexやClaude Codeには入りません。
...
いずれも小規模な架空例を用いた開発時の評価です。全モデル・全原稿での品質や、スキルなしに対する優越性は保証しません。
