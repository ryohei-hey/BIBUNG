...
検証済みの項目と未検証の項目は、[評価結果](evals/RESULTS.md)の冒頭の一覧表に日付付きでまとめています。v0.1.0ではCodex CLIでの発見・呼び出し・補助資料の読み込みを、v0.2.0ではClaude Code CLIでの同じ項目と、独自作成した架空例による改稿の比較を確認しました。macOS／LinuxでのCI実行はGitHub Actionsのワークフローに含めていますが、リモートでの実行結果はこのリポジトリを公開して初めて得られます。
